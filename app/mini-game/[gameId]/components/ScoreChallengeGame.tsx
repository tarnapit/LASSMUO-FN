...
interface ScoreChallengeGameProps {
  onGameFinish: (result: GameResult) => void;
}

export default function ScoreChallengeGame({ onGameFinish }: ScoreChallengeGameProps) {
  ...
  // Initialize game
  useEffect(() => {
    const gameQuestions = getQuestionsForMode('score-challenge');
  ...
  const handleAnswerSelect = useCallback((answer: any) => {
    ...
    if (isCorrect) {
      ...
      questionBonus = calculateTimeBonus(timeSpent, currentQuestion.timeBonus || 0);
    }
    ...
    setBonusPoints(prev => prev + questionBonus);
  ...
  }, [currentQuestion, currentQuestionIndex, isAnswered, questions.length, questionStartTime]);
      ...
      <div className="space-y-6">
        ...
        {!isAnswered && (
          <button
            ...
            className="w-full bg-gradient-to-r from-blue-500 to-purple-500 text-white font-bold py-3 px-6 rounded-xl hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            ส่งคำตอบ
          </button>
        )}
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white">โหมดสะสมคะแนน</h1>
                <p className="text-gray-400">ตอบคำถามเพื่อสะสมคะแนน</p>
              </div>
            </div>
            
            <div className="text-right">
              <div className="text-2xl font-bold text-yellow-400">{score + bonusPoints}</div>
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-3xl p-8 border border-white/20 mb-8">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              {currentQuestion.timeBonus && (
                <div className="text-right">
                  <div className="text-purple-400 text-sm flex items-center gap-1">
                    <Zap size={12} />
                    โบนัส +{currentQuestion.timeBonus}
                  </div>
                  <div className="text-gray-400 text-xs">ตอบเร็วได้โบนัส</div>
                </div>
              )}
            </div>
            ...
            {showResult && currentQuestion.explanation && (
              <div className="mt-6 p-4 bg-blue-500/10 rounded-xl border border-blue-500/30">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-blue-400 font-bold mb-2">คำอธิบาย</h4>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white/5 rounded-2xl p-4 text-center border border-white/10">
              ...
              <div className="text-gray-400 text-sm">คำถามปัจจุบัน</div>
            </div>
            
            <div className="bg-white/5 rounded-2xl p-4 text-center border border-white/10">
              <div className="text-2xl font-bold text-yellow-400">{score}</div>
              <div className="text-gray-400 text-sm">คะแนนหลัก</div>
            </div>
            
            <div className="bg-white/5 rounded-2xl p-4 text-center border border-white/10">
              <div className="text-2xl font-bold text-purple-400">{bonusPoints}</div>
              <div className="text-gray-400 text-sm">คะแนนโบนัส</div>
            </div>
            
            <div className="bg-white/5 rounded-2xl p-4 text-center border border-white/10">
              <div className="text-2xl font-bold text-green-400">{score + bonusPoints}</div>
              <div className="text-gray-400 text-sm">คะแนนรวม</div>
            </div>
          </div>
        </div>
      </div>
